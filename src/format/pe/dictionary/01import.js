export async function parse_pe_import(parser, dictionary) {
    ...
    for (let index = 0; ; index++) {
        const view = await parser.pointerToView(dictionary.VritualAddress + index * 20, 20)
        const result = {}
        result.Index = index
...
async function import_list_real(parser, view, bits) {
    const itemList = []
    for (let index = 0; ; index++) {
        const union = view.getUint32(index * bits, true)
        if (union > 0) {
            const item = {}
            item.Index = index
            // if (union >>> 31 > 0) {
